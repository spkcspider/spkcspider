...
class SpiderTagForm(forms.Form):
    ...
    def get_info(self):
        return "verifiers=%s;tag=%s;" % (
            base64.b64_urlencode(self.cleaned_data["url"]),
    ...
    def is_valid(self):
        ...
        if self.instance.associated and \
           self.instance.associated.usercomponent == self.usercomponent:
            maxamount = 1
        else:
            maxamount = 0
        return len(SpiderTag.objects.filter(
            info__contains="type=Tag;"
        ).filter(info__contains="burl=%s" % burl)) <= maxamount
