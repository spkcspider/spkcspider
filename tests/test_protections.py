...
class TravelProtectionTest(TransactionWebTest):
    ...
    def test_fake_wipe_user(self):
        index = self.user.usercomponent_set.filter(name="index").first()
        index_count = index.contents.count()
        testu = self.user.usercomponent_set.create(name="test")
        createurl = reverse(
            "spider_base:ucontent-add",
            kwargs={
                "token": index.token,
        ...
        form.set("protect_components", (testu.name,))
        form.set("master_pw", "abc")
        response = form.submit().follow()
        self.assertEqual(response.status_code, 200)
        ...
        self.assertEqual(
            index.contents.count(), index_count+2
        )
        ...
        self.assertFalse(
            ...
            ).exclude(strength__gte=9).exclude(name=testu.name)
        )
        self.assertTrue(
