...
urlpatterns = [
    ...
    url(r'^(?P<user>[-\w]+)/update/(?P<id>[-\w]+)/$', BrokerUpdate.as_view(), name='bk-update'),
    url(r'^(?P<user>[-\w]+)/view/(?P<id>[-\w]+)/$', BrokerDetail.as_view(), name='bk-view'),
    #url(r'^bk/(?P<user>[-\w]+)/json/(?P<id>[-\w]+)/$', BrokerJson.as_view(), name='bk-json'),
    url(r'^(?P<user>[-\w]+)/delete/(?P<id>[-\w]+)/$', BrokerDelete.as_view(), name='bk-delete'),
